...
contract PriceFeed is IPriceFeed {
    ...
    function getLatestPrice() external view override returns (int, uint) {
        ...
        return (int(getLatestPriceTSLA()), block.timestamp);
       }

        function getLatestPriceTSLA() public view returns (uint) {
            (, int price, , , uint timestamp) = priceFeedTSLA.latestRoundData();
            return uint(price);
        }
}